...
sudo tee /etc/mysql/mysql.conf.d/mysqld-acore.cnf < "$SCRIPT_DIR/config/mysqld-acore.cnf"
...
for file in data/sql/create/create_mysql.sql "$SCRIPT_DIR/config/create_db.sql"; do
	cat "$file"
	sudo mysql < "$file"
done

header "Installing services"
for file in $SCRIPT_DIR/config/systemd/*.service; do
	step "$(basename "$file")"
